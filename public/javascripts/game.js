...
//    $(mapEl).parents("#container").addClass(data.player.race.toLowerCase());
    ...
    this.selection = new Selection(this.map, "green", this.selectionListener);
    ...
    unitScriptNames.filter(function(s) { return s != ''; }).forEach(function(u) {
        ResourcePreloader.add(Game.getUnitTypeImage(u, this.map.tileset.name));
    }, this);
