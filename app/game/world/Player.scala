...
class Player(val race: Race, val control: Control, startingRes: (Int, Int, Int), val mapWidth: Int, val mapHeight: Int,
  ...
  var seenPositions: Set[TileVisibility] = computeSeenPositions

  def computeSeenPositions: Set[TileVisibility] = {
    units
      // todo handle
      //  1 1 4      1 1 4
      //  1 4 4  =>  1 1 4
  ...
  def unitById(id: Int): Option[game.unit.Unit] = units.find { _.id == id }
}
