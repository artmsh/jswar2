...
object PudCodec {
  ...
  implicit val pud = {
    toTupleCodec(sectionHeader, typeSection) ::
    toTupleCodec(sectionHeader, verSection) ::
    toTupleCodec(sectionHeader, descSection) ::
    toTupleCodec(sectionHeader, ownrSection) ::
    toTupleCodec(sectionHeader, eraSection) ::
    toTupleCodec(sectionHeader, dimSection)

  }.as[Pud]

  def toTupleCodec[A, B](a: Codec[A], b: Codec[B]): Codec[(A, B)] = {
    a.flatZip[B](_ => b)
  }
}
