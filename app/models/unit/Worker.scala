...
abstract class Worker extends Troop with Movable with Attackable with GoldGatherable with LumberGatherable {
...
class Peasant extends Worker { type T = Human.type }
class Peon extends Worker { type T = Orc.type }